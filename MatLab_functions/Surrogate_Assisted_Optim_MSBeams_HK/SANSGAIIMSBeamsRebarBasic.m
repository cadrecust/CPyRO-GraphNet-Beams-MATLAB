function [extrOptPFCS,PF_CFA,PF_VOL,newPop,feasibleSol,genCFA,genVOL,...
    IGDt,IGDv]=SANSGAIIMSBeamsRebarBasic(b,h,span,brec,hrec,hagg,pmin,pmax,rebarAvailable,...
...
for i =1:pop_size % can be set to parfor
    ...
    cc(i,1)]=SAMOOptimMSFSBeamsRebarBasic(b,h,span,brec,hrec,hagg,pmin,pmax,...
...
for gen_count=1:gen_max
    ...
    for ii = 1:nchildren % can be set to parfor
        ...
        DistrRebarLeft{ii},RebarDistrMid{ii},DistrRebarRight{ii},NbCombo9Span(:,:,ii),...
        totalnbSpan(:,:,ii),CSrebarSpans(ii,:),nbcut3sec(:,:,ii),nblowLeft(:,:,ii),...
        nbTopMid(:,:,ii),nblowRight(:,:,ii),bestCS(ii),cc(ii,1)]=...
        SAMOOptimMSFSBeamsRebarBasic(b,h,span,brec,hrec,hagg,pmin,pmax,sepMin,...
        fcu,load_conditions,fy,cutLoc,Aos3,Wfac,dbc);
